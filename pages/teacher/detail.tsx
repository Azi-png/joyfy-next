...
import ReviewCard from '../../libs/components/teacher/ReviewCard';
...
const TeacherDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const user = useReactiveVar(userVar);
	const [teacherId, setTeacherId] = useState<string | null>(null);
	const [teacher, setTeacher] = useState<Member | null>(null);
	...
	const {
		...
		variables: { input: teacherId },
		skip: !teacherId,
		onCompleted: (data) => {
			setTeacher(data?.getMember);
			setSearchFilter({
				...searchFilter,
				search: {
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			setTeacherComments(data?.getComments?.list);
	...
	/** LIFECYCLES **/
	useEffect(() => {
		if (router.query.teacherId) setTeacherId(router.query.teacherId as string);
	}, [router]);
	...
	const createCommentHandler = async () => {
		try {
			...
			if (user._id === teacherId) throw new Error('Cannot write a review for yourself');
	...
	if (device === 'mobile') {
		return <div>TEACHER DETAIL PAGE MOBILE</div>;
	} else {
		return (
			<Stack className={'teacher-detail-page'}>
				<Stack className={'container'}>
					<Stack className={'teacher-info'}>
						<img
							src={
								teacher?.memberImage ? `${REACT_APP_API_URL}/${teacher?.memberImage}` : '/img/profile/defaultUser.svg'
							}
							alt=""
						/>
						<Box
							component={'div'}
							className={'info'}
							onClick={() => redirectToMemberPageHandler(teacher?._id as string)}
						>
							<strong>{teacher?.memberFullName ?? teacher?.memberNick}</strong>
							<div>
								<img src="/img/icons/call.svg" alt="" />
								<span>{teacher?.memberPhone}</span>
							</div>
						</Box>
					</Stack>
					<Stack className={'teacher-home-list'}>
						...
						<Stack className={'pagination'}>
							{courseTotal ? (
								<>
									...
									<span>
										Total {courseTotal} course{courseTotal > 1 ? 's' : 'y'} available
									</span>
								</>
							) : (
					...
					<Stack className={'review-box'}>
						...
						{commentTotal !== 0 && (
							<Stack className={'review-wrap'}>
								...
								{teacherComments?.map((comment: Comment) => {
...
export default withLayoutBasic(TeacherDetail);
