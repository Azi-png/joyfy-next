...
import { CourseLocation, CourseStatus } from '../../../libs/enums/course.enum';
	...
	const tabChangeHandler = async (event: any, newValue: string) => {
		...
		switch (newValue) {
			case 'ACTIVE':
				...
				break;
			case 'SOLD':
				setCoursesInquiry({ ...coursesInquiry, search: { courseStatus: CourseStatus.CANCELLED } });
				break;
			case 'DELETE':
			...
			if (newValue !== 'ALL') {
				setCoursesInquiry({
					...
					search: {
						...
						courseLocationList: [newValue as CourseLocation],
					},
				});
			} else {
				delete coursesInquiry?.search?.courseLocationList;
				setCoursesInquiry({ ...coursesInquiry });
			}
		} catch (err: any) {
						...
						<Box component={'div'}>
							...
							<Stack className={'search-area'} sx={{ m: '24px' }}>
								<Select sx={{ width: '160px', mr: '20px' }} value={searchType}>
									...
									{Object.values(CourseLocation).map((location: string) => (
