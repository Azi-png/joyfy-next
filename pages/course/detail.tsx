...
const CourseDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [destinationCourses, setDestinationCourses] = useState<Course[]>([]);
	...
	const {
		...
		onCompleted: (data: T) => {
			...
			if (data?.getCourse) setSlideImage(data?.getCourse?.courseImages[0]);
		},
	});
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: course?.courseLocation ? [course?.courseLocation] : [],
				},
			},
		},
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getCourses?.list) setDestinationCourses(data?.getCourses?.list);
		},
	});
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getComments?.list) setCourseComments(data?.getComments?.list);
	...
	const likeCourseHandler = async (user: T, id: string) => {
		try {
			...
			await getCourseRefetch({ input: id });
			await getCoursesRefetch({
				input: {
					...
					direction: Direction.DESC,
					search: {
						locationList: [course?.courseLocation],
					},
				},
			});
		...
		return (
			<div id={'course-detail-page'}>
				<div className={'container'}>
					<Stack className={'course-detail-config'}>
						<Stack className={'course-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{course?.courseTitle}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{course?.courseLocation}</Typography>
										...
										<Stack className={'buy-rent-box'}>
											...
											{course?.isOffline && (
												<>
													...
													<Typography className={'buy-rent'}>Offline</Typography>
												</>
											)}
										</Stack>
										<Stack className={'divider'}></Stack>
										...
										<Typography className={'date'}>{moment().diff(course?.createdAt, 'days')} days ago</Typography>
									</Stack>
									<Stack className={'bottom-box'}>
										...
										<Stack className="option">
											<Typography>Duration: {course?.courseDuration} minute </Typography>
										</Stack>
										<Stack className={'divider'}></Stack>
								...
								<Stack className={'right-box'}>
									<Stack className="buttons">
										<Stack className="button-box">
											...
											<Typography>{course?.courseViews}</Typography>
										</Stack>
										<Stack className="button-box">
											...
											<Typography>{course?.courseLikes}</Typography>
										</Stack>
									</Stack>
									<Typography>${formatterStr(course?.coursePrice)} per class</Typography>
								</Stack>
							</Stack>
							<Stack className={'images'}>
								...
								<Stack className={'sub-images'}>
									{course?.courseImages.map((subImg: string) => {
						...
						<Stack className={'course-desc-config'}>
							<Stack className={'left-config'}>
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{course?.courseAge}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{course?.courseDuration}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											<Typography className={'title'}>Year created</Typography>
											<Typography className={'option-data'}>{moment(course?.createdAt).format('YYYY')}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
									...
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{course?.courseType}</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										<Typography className={'title'}>Course Description</Typography>
										<Typography className={'desc'}>{course?.courseDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>Course Details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Price</Typography>
													<Typography className={'data'}>${formatterStr(course?.coursePrice)} per course</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
												...
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Duration</Typography>
													<Typography className={'data'}>{course?.courseDuration}min</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Age</Typography>
													<Typography className={'data'}>{course?.courseAge}</Typography>
												</Box>
											</Stack>
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Year started</Typography>
													<Typography className={'data'}>{moment(course?.createdAt).format('YYYY')}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Course Type</Typography>
												...
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Course weeks</Typography>
													<Typography className={'data'}>{course?.courseDurationWeeks}</Typography>
												</Box>

												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>per week</Typography>
													<Typography className={'data'}>{course?.coursesPerWeek}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Course Times</Typography>
													<Box className={'course-times-list'}>
														{course?.courseTimes?.map((courseTime, index) => (
															<Typography key={index} className={'data'}>
																{courseTime.day}: {courseTime.time}
																{index < course.courseTimes.length - 1 && ', '}
															</Typography>
														))}
													</Box>
												</Box>
												...
												{/* <Box component={'div'} className={'info'}>
													...
													<Typography className={'data'}>
														For {property?.isOnline && 'Online'} {property?.isOffline && 'Offline'}
													</Typography>
												</Box> */}
											</Stack>
										</Stack>
								...
								{/* <Stack className={'floor-plans-config'}>
									<Typography className={'title'}>Photo from class</Typography>
									<Stack className={'image-box'}>
										<img src={'/img/property/photo.jpg'} alt={'image'} />
									</Stack>
								</Stack> */}
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{courseComments?.map((comment: Comment) => {
									...
									<Stack className={'image-info'}>
										...
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${course?.memberData?._id}`}>
												<Typography className={'name'}>{course?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{course?.memberData?.memberPhone}</Typography>
											</Stack>
						...
						{destinationCourses.length !== 0 && (
							<Stack className={'similar-courses-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Other courses</Typography>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationCourses.map((course: Course) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={course.courseTitle}>
													<CourseBigCard course={course} likeCourseHandler={likeCourseHandler} key={course?._id} />
												</SwiperSlide>
											);
										})}
									</Swiper>
