...
import CourseCard from '../../libs/components/course/CourseCard';
...
const CourseList: NextPage = ({ initialInput, ...props }: any) => {
	...
	if (device === 'mobile') {
		...
		return (
			<div id="course-list-page" style={{ position: 'relative' }}>
							...
							<Stack className={'list-config'}>
								{courses?.length === 0 ? (
									...
									courses.map((course: Course) => {
										return <CourseCard course={course} likeCourseHandler={likeCourseHandler} key={course?._id} />;
									})
								)}
							</Stack>
							<Stack className="pagination-config">
								...
								{courses.length !== 0 && (
									<Stack className="total-result">
										<Typography>
											Total {total} course{total > 1 ? 's' : 'y'} available
										</Typography>
									</Stack>
								)}
							</Stack>
