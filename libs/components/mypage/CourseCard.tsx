...
export const CourseCard = (props: CourseCardProps) => {
	...
	if (device === 'mobile') {
		...
		return (
			<Stack className="course-card-box">
				<Stack className="image-box" onClick={() => pushCourseDetail(course?._id)}>
					<img src={`${process.env.REACT_APP_API_URL}/${course.courseImages[0]}`} alt="" />
				</Stack>
				<Stack className="information-box" onClick={() => pushCourseDetail(course?._id)}>
					...
					<Typography className="address">{course.courseAddress}</Typography>
					<Typography className="price">
						<strong>${formatterStr(course?.coursePrice)}</strong>
					</Typography>
				</Stack>
				<Stack className="date-box">
					<Typography className="date">
						<Moment format="DD MMMM, YYYY">{course.createdAt}</Moment>
					</Typography>
				</Stack>
				...
				{!memberPage && course.courseStatus !== 'CANCELLED' && (
					<Menu
						anchorEl={anchorEl}
						open={open}
						onClose={handleClose}
						...
						{course.courseStatus === 'ACTIVE' && (
							<>
								<MenuItem
									...
									onClick={() => {
										handleClose();
										updateCourseHandler(CourseStatus.CANCELLED, course?._id);
									}}
								>
									Cancel
								</MenuItem>
				...
				<Stack className="views-box">
					<Typography className="views">{course.courseViews.toLocaleString()}</Typography>
				</Stack>
				{!memberPage && course.courseStatus === CourseStatus.ACTIVE && (
