...
import { CourseFormat, CourseLocation, CourseTime, CourseType, DaysOfWeek } from '../../enums/course.enum';
import { REACT_APP_API_URL, courseFormat } from '../../config';
...
const AddCourse = ({ initialValues, ...props }: any) => {
	...
	const [courseFormats] = useState<CourseFormat[]>(Object.values(CourseFormat));
	...
	useEffect(() => {
		setInsertCourseData({
			...insertCourseData,
			courseTitle: getCourseData?.getCourse ? getCourseData?.getCourse?.courseTitle : '',
			coursePrice: getCourseData?.getCourse ? getCourseData?.getCourse?.coursePrice : 0,
			courseType: getCourseData?.getCourse ? getCourseData?.getCourse?.courseType : '',
			courseLocation: getCourseData?.getCourse ? getCourseData?.getCourse?.courseLocation : '',
			isOnline: getCourseData?.getCourse ? getCourseData?.getCourse?.isOnline : false,
			isOffline: getCourseData?.getCourse ? getCourseData?.getCourse?.isOffline : false,
			courseDuration: getCourseData?.getCourse ? getCourseData?.getCourse?.courseDuration : 0,
			courseAge: getCourseData?.getCourse ? getCourseData?.getCourse?.courseAge : 0,
			courseFormat: getCourseData?.getCourse ? getCourseData?.getCourse?.courseFormat : CourseFormat.GROUP_CLASS,
			courseDesc: getCourseData?.getCourse ? getCourseData?.getCourse?.courseDesc : '',
			courseImages: getCourseData?.getCourse ? getCourseData?.getCourse?.courseImages : [],
			courseTimes: getCourseData?.getCourse ? getCourseData?.getCourse?.courseTimes : [],
			courseDurationWeeks: getCourseData?.getCourse ? getCourseData?.getCourse?.courseDurationWeeks : 0,
			coursesPerWeek: getCourseData?.getCourse ? getCourseData?.getCourse?.coursesPerWeek : 0,
			startDate: getCourseData?.getCourse ? getCourseData?.getCourse?.startDate : null,
		});
	}, [getCourseLoading, getCourseData]);

	/** HANDLERS **/
	// 3. CourseTime qo'shish funksiyasi
	const addCourseTime = () => {
		const newCourseTime = {
		...
		setInsertCourseData({
			...insertCourseData,
			courseTimes: [...insertCourseData.courseTimes, newCourseTime],
		});
	};

	// 4. CourseTime o'chirish funksiyasi
	const removeCourseTime = (index: number) => {
		const updatedCourseTimes = insertCourseData.courseTimes.filter((_, i) => i !== index);
		setInsertCourseData({
			...insertCourseData,
	...
	// 5. CourseTime yangilash funksiyasi
	const updateCourseTime = (index: number, field: 'day' | 'time', value: string) => {
		const updatedCourseTimes = insertCourseData.courseTimes.map((courseTime, i) =>
			i === index ? { ...courseTime, [field]: value } : courseTime,
		);
		setInsertCourseData({
			...insertCourseData,
			courseTimes: updatedCourseTimes,
	...
	async function uploadImages() {
		try {
			...
			setInsertCourseData({ ...insertCourseData, courseImages: responseImages });
		} catch (err: any) {
	...
	const doDisabledCheck = () => {
		if (
			...
			insertCourseData.courseLocation === '' || // @ts-ignore
			insertCourseData.courseAddress === '' || // @ts-ignore
			// insertCourseData.isOnline === '' || // @ts-ignore
			// insertCourseData.isOffline === '' ||
			insertCourseData.courseDuration === 0 ||
			insertCourseData.courseAge === 0 ||
			!insertCourseData.courseFormat ||
			insertCourseData.courseDesc === '' ||
			insertCourseData.courseImages.length === 0
		) {
			return true;
		}
	};
	...
	const insertCourseHandler = useCallback(async () => {
		try {
			...
			await sweetMixinSuccessAlert('This course has been created successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myCourses',
	...
	const updateCourseHandler = useCallback(async () => {
		try {
			...
			await sweetMixinSuccessAlert('This course has been updated successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myCourses',
	...
	if (device === 'mobile') {
		return <div>ADD NEW COURSE MOBILE PAGE</div>;
	} else {
		return (
			<div id="add-course-page">
				<Stack className="main-title-box">
					<Typography className="main-title">Add New Course</Typography>
				...
				<div>
					<Stack className="config">
						<Stack className="description-box">
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										onChange={({ target: { value } }) =>
											setInsertCourseData({ ...insertCourseData, coursePrice: parseInt(value) })
										}
									/>
								</Stack>
								<Stack className="price-year-after-price">
									...
									<select
										...
										defaultValue={insertCourseData.courseType || 'select'}
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										defaultValue={insertCourseData.courseLocation || 'select'}
										value={insertCourseData.courseLocation || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertCourseData({ ...insertCourseData, courseLocation: value })
										}
									>
										<>
											<option selected={true} disabled={true} value={'select'}>
											...
											{courseLocation.map((location: any) => (
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										className="description-input"
										placeholder={'Address'}
										value={insertCourseData.courseAddress}
										onChange={({ target: { value } }) =>
											setInsertCourseData({ ...insertCourseData, courseAddress: value })
										}
									/>
								</Stack>
							</Stack>
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									<Typography className="title">isOnline</Typography>
									<select
										className={'select-description'}
										value={insertCourseData.isOnline ? 'yes' : 'no'}
										defaultValue={insertCourseData.isOffline ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertCourseData({ ...insertCourseData, isOnline: value === 'yes' })
										}
									>
								...
								<Stack className="price-year-after-price">
									<Typography className="title">isOffline</Typography>
									<select
										className={'select-description'}
										value={insertCourseData.isOffline ? 'yes' : 'no'}
										defaultValue={insertCourseData.isOffline ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertCourseData({ ...insertCourseData, isOffline: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertCourseData.courseFormat || 'select'}
										onChange={({ target: { value } }) =>
											setInsertCourseData({
												...insertCourseData,
												courseFormat: value as CourseFormat, // ✅ Cast string to CourseFormat enum
											})
										}
									>
										<option selected={true} disabled={true} value={'select'}>
										...
										{courseFormats.map((format: any) => (
											<option value={format} key={format}>
												{format.replace(/_/g, ' ')} {/* This makes "GROUP_CLASS" display as "GROUP CLASS" */}
											</option>
										))}
									</select>
									<div className={'divider'}></div>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price" style={{ marginBottom: '20px' }}>
									<Typography className="title">Course duration</Typography>
									<input
										type="text"
										className="select-description"
										placeholder="Enter course duration"
										value={insertCourseData.courseDuration || ''}
										onChange={({ target: { value } }) =>
											setInsertCourseData({
												...insertCourseData,
												courseDuration: value ? parseInt(value) || 0 : 0,
											})
										}
									/>
								</Stack>
								{/* <Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertPropertyData.courseDuration || 'select'}
										defaultValue={insertPropertyData.courseDuration || 'select'}
										onChange={({ target: { value } }) =>
											setInsertPropertyData({ ...insertPropertyData, courseDuration: parseInt(value) })
										}
									>
								...
								<Stack className="price-year-after-price">
									<Typography className="title">Course Age</Typography>
									<select
										className={'select-description'}
										value={insertCourseData.courseAge || 'select'}
										defaultValue={insertCourseData.courseAge || 'select'}
										onChange={({ target: { value } }) =>
											setInsertCourseData({ ...insertCourseData, courseAge: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
										{[3, 4, 5, 6, 7, 8, 9].map((bed: number) => (
								...
								{/* <Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertPropertyData.courseAge || 'select'}
										defaultValue={insertPropertyData.courseAge || 'select'}
										onChange={({ target: { value } }) =>
											setInsertPropertyData({ ...insertPropertyData, courseAge: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
										</option>
										{[50, 100, 150, 200, 250, 300, 350, 400, 450, 500].map((square: number, index) => (
											<option value={`${square}`} key={index}>
												{square}
											</option>
										))}
									</select>
							...
							<Stack className="price-year-after-price" style={{ marginBottom: '30px' }}>
								<Typography className="title">Courses Per Week</Typography>
								<input
									type="text"
									className="select-description"
									style={{
										marginTop: '20px',
										width: '300px', // yoki '100%', '50%'
										height: '45px',
									}}
									placeholder="How many courses per week"
									value={insertCourseData.coursesPerWeek || ''}
									onChange={({ target: { value } }) =>
										setInsertCourseData({
											...insertCourseData,
											coursesPerWeek: value ? parseInt(value) || 0 : 0,
										})
									}
								/>
							</Stack>
							<Stack className="price-year-after-price">
								<Typography className="title">Course Duration (weeks)</Typography>
								<input
									type="text"
									className="select-description"
									style={{
									...
									value={insertCourseData.courseDurationWeeks || ''}
									onChange={({ target: { value } }) =>
										setInsertCourseData({
											...insertCourseData,
											courseDurationWeeks: value ? parseInt(value) || 0 : 0,
										})
									}
								/>
							</Stack>
							<Typography className="course-title">Course Schedule</Typography>
							<Stack className="course-times-section">
								<Stack className="config-row">
									<Typography className="title">Course Times</Typography>
									<Button
									...
									>
										<Typography>+ Add Time Slot</Typography>
									</Button>
								</Stack>

								{insertCourseData.courseTimes.map((courseTime: any, index: number) => (
									<Stack
										key={index}
										className="config-row"
										style={{ marginBottom: '15px', padding: '15px', border: '1px solid #ddd', borderRadius: '8px' }}
									>
										<Stack className="price-year-after-price">
											<Typography className="title">Day</Typography>
											<select
												className="select-description"
												value={courseTime.day}
												onChange={({ target: { value } }) => updateCourseTime(index, 'day', value)}
											>
										...
										<Stack className="price-year-after-price">
											...
											<input
												type="time"
												className="description-input"
												value={courseTime.time}
												onChange={({ target: { value } }) => updateCourseTime(index, 'time', value)}
											/>
										</Stack>

										<Stack className="price-year-after-price" style={{ alignItems: 'center' }}>
											<Button
								...
								{insertCourseData.courseTimes.length === 0 && (
									<Stack
										style={{ padding: '20px', textAlign: 'center', border: '2px dashed #ddd', borderRadius: '8px' }}
									>
										<Typography style={{ color: '#666' }}>
											Hozircha vaqt jadval qo'shilmagan. "Add Time Slot" tugmasini bosing.
										</Typography>
									</Stack>
								)}
							</Stack>

							<Typography className="course-title">Course Description</Typography>
							<Stack className="config-column">
						...
						<Typography className="upload-title">Upload photos of your course</Typography>
						<Stack className="images-box">
							...
							<Stack className="gallery-box">
								{insertCourseData?.courseImages.map((image: string) => {
						...
						<Stack className="buttons-row">
							{router.query.courseId ? (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={updateCourseHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							) : (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={insertCourseHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							)}
						</Stack>
...
AddCourse.defaultProps = {
	initialValues: {
		...
		courseDuration: 0,
		courseAge: 0,
		courseFormat: CourseFormat.GROUP_CLASS,
		courseDesc: '',
		courseImages: [],
		// Qolib ketgan maydonlar:
		courseTimes: [],
		courseDurationWeeks: 0,
		coursesPerWeek: 0,
		startDate: null,
	},
};
