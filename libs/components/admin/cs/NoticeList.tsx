...
export const NoticeList = ({ notices, loading = false, page, limit, refetch, onEditClick }: NoticeListProps) => {
									...
									<TableRow hover key={notice._id ? notice._id.toString() : undefined} selected={selected}>
										...
										<TableCell>
											...
											<Tooltip title={isDeleted ? 'Delete permanently' : 'Move to Deleted'}>
												<IconButton
													onClick={() =>
														isDeleted ? handlePermanentDelete(notice._id as string) : handleDelete(notice._id as string)
													}
												>
													<DeleteRoundedIcon />
												</IconButton>
