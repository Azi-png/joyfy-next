...
interface EnhancedTableProps {
	numSelected: number;
	onRequestSort: (event: React.MouseEvent<unknown>, course: keyof Data) => void;
					...
					<TableBody>
						...
						{courses.length !== 0 &&
							courses.map((course: Course, index: number) => {
								const courseImage = `${REACT_APP_API_URL}/${course?.courseImages[0]}`;
								...
								return (
									<TableRow hover key={course?._id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
										...
										<TableCell align="center">{course.courseLocation}</TableCell>
